...
从零开始可先读第 1–3 节，再按效果需要查阅第 4–10 节。可复制源文件见 [完整示例目录](Content/Examples/README.zh-CN.md)，玩家构筑与疲劳规则见 [客户端功能与操作](GameplayGuide.zh-CN.md)，素材制作和编辑器操作见 [开发工具与内容制作](../tools/README.zh-CN.md)。
...
手牌满时，真实抽牌会被烧毁；回手因容量不足失败时，原实体死亡，并触发死亡与普通离场效果。抽空牌库后继续抽牌遵守协议中的疲劳规则，包括帧末判定的“疲劳即死”；生成不触发疲劳。详见 [抽牌、疲劳与满手](GameplayGuide.zh-CN.md#抽牌疲劳与满手)。
